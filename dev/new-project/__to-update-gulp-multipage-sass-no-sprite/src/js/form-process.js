...
const tmplPath = '../'
const sounds = {
  success: `${tmplPath}php/audio/success.mp3`,
  error: `${tmplPath}php/audio/error-1.mp3`,
};
...
forms.forEach((form) => {
  ...
  // Защита: если в форме нет input[type="file"], пропускаем
  if (!inputTypeFile) {
    return;
  }
  ...
  let requiredInputs = form.querySelectorAll('[data-required]')
  requiredInputs.forEach((requiredInput) => {
    requiredInput.oninput = function() {
      this.classList.remove('not-valid')
    }
  })

  form.addEventListener("submit", function (event) {
    send(event, `${tmplPath}php/send-order.php`);
  });
    ...
    const validate = function() {
      ...
      requiredInputs.forEach((requiredInput)=>{
        ...
        if(requiredInput.value.trim() === '') {
          requiredInput.value = ''
          requiredInput.classList.add('not-valid');
          isValid = false
          if(requiredInput.name === "userName") {
            requiredInput.placeholder = 'Укажите ваше имя'
          }
          if(requiredInput.name === "userPhone") {
            requiredInput.placeholder = 'Укажите ваш телефон'
          }
          if(requiredInput.name === "userEmail") {
            requiredInput.placeholder = 'Укажите ваш email'
          }
        }
      })
      return isValid;
