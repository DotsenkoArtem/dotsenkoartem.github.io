...
for (let i = 0; i < cards3D.length; i++) {
  const card = cards3D[i];
  const cardInner = card.querySelector(".card__inner");
  const cardCoords = card.getBoundingClientRect();
  const cardCenter = {
    x: cardCoords.right - cardCoords.width / 2,
    y: cardCoords.bottom + window.pageYOffset - cardCoords.height / 2,
  };

  card.fixed = false;
  const maxHypotenuse = Math.sqrt(
    Math.pow(cardCoords.width / 2, 2) + Math.pow(cardCoords.height / 2, 2)
  );
  ...
  function calcCardTransform(e) {
    ...
    cardCurrentTransform =
      "rotate3d(" +
      -cardCenterOffsetY / (cardCoords.height / 2) +
      ", " +
      cardCenterOffsetX / (cardCoords.width / 2) +
      ", 0, " +
      (
        (Math.sqrt(
          Math.pow(cardCenterOffsetX, 2) + Math.pow(cardCenterOffsetY, 2)
        ) /
          maxHypotenuse) *
        45
      ).toFixed(2) +
    ...
    console.log([cardCenterOffsetX, cardCenterOffsetY]);
    console.log(maxHypotenuse);
  }
}
